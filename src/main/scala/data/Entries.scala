...
class Entries {
  ...
  def add(entryToAdd:Entry){
    ...
    this.entries.synchronized{
      try{
        entries = entries++Map(entryToAdd.getID() -> (entries(entryToAdd.getID())++Set(entryToAdd) ) )
      } 
      catch{
        case e:NoSuchElementException=>entries = entries++Map(entryToAdd.getID() -> Set(entryToAdd ) )
      }
      this.status = true;
    }   
  }
  ...
  def remove(entryToRemove:Entry){
    ...
    this.entries.synchronized{
      if(entries.contains(entryToRemove.getID())){
        var temp:Set[Entry] = entries(entryToRemove.getID())--Set(entryToRemove)
        entries = entries++( if(temp.size>0) Map(entryToRemove.getID()->temp) else Map() )
        this.status = true;
      }     
    }    
  }
